...
The code, all stored experiment outputs, and the complete commands to
reproduce every figure and table in this report are available in the
accompanying repository; see its \texttt{README} for step-by-step
instructions.
...
label-based orders, with one heatmap per seed
(Fig.~\ref{fig:app-heatmaps}): each individual run specializes to a single
class rather than learning a balanced classifier, and the selected class
can differ between seeds, which a seed-averaged heatmap would hide.
...
\begin{figure}[!h]
  ...
  orders at learning rate $0.05$, shown separately for each seed.
  Every run collapses onto a single class;
  \texttt{label\_block\_random} selects class 0 for seeds 0 and 1 but a
  different class for seed 2.}
